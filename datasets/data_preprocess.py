    ...
    for key in file1_json.keys():
        for item_key in file1_json[key]["payload"].keys():
            if len(file1_json[key]["payload"][item_key]) > 100:
                count += 1
                result_samples["0"]["payload"][str(count)] = file1_json[key]["payload"][
                    item_key
                ]
    ...
    for key in file2_json.keys():
        for item_key in file2_json[key]["payload"].keys():
            if len(file2_json[key]["payload"][item_key]) > 100:
                count += 1
                result_samples["1"]["payload"][str(count)] = file2_json[key]["payload"][
                    item_key
                ]
    file2_random_samples = random.sample(result_samples["1"]["payload"].keys(), 5000)

    combined_result = {
        "0": {"samples": len(file1_random_samples), "payload": {}},
        "1": {"samples": len(file2_random_samples), "payload": {}},
    }
    count = 0
...
def copy_dir_tls13_data():
    ...
    for label_index in range(len(label_name_list)):
        ...
        print(
            ...
            % (label_name_list[label_index], src_dir, dst_dir)
        )
...
def basic_process_1(y_dict, x_dict):
    ...
    x_result = {
        "len_train": x_len_train_np,
        "len_test": x_len_test_np,
        "len_valid": x_len_valid_np,
        "time_train": x_time_train_np,
        "time_test": x_time_test_np,
        "time_valid": x_time_valid_np,
    }
    return x_result
...
def time_process_1(time_data):
    ...
    time_data_new = [0.0]
    time_data_new.extend(
        [
            time_data[time_index] - time_data[time_index - 1]
            for time_index in range(1, len(time_data))
        ]
    )
