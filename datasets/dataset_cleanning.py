...
def deal_finetuning(excluding_label):
    ...
    while label_number > 105:
        for index in range(len(train_data)):
            ...
            if str(label_number) + "\t" in data:
                new_data = data.replace(
                    str(label_number) + "\t", str(excluding_label[count]) + "\t"
                )
                train_data[index] = new_data

        for index in range(len(valid_data)):
            if str(label_number) + "\t" in valid_data[index]:
                new_data = valid_data[index].replace(
                    str(label_number) + "\t", str(excluding_label[count]) + "\t"
                )
        ...
        for index in range(len(test_data)):
            if str(label_number) + "\t" in test_data[index]:
                new_data = test_data[index].replace(
                    str(label_number) + "\t", str(excluding_label[count]) + "\t"
                )
    ...
    deal_result = input(
        "please delete the last blank line in %s and input '1'"
        % (save_dataset_path + "test_dataset.tsv")
    )
    if deal_result == "1":
...
def deal_test(excluding_label):
    ...
    deal_result = input(
        "please delete the last blank line in %s and input '1'"
        % (dataset_path + "new_106\\test_dataset.tsv")
    )
    if deal_result == "1":
