...
pcap_path, dataset_save_path, samples, features, dataset_level = (
    "I:\\cstnet-tls1.3\\packet\\splitcap\\",
    "I:\\cstnet-tls1.3\\packet\\result\\",
    [5000],
    ["payload"],
    "packet",
)
...
def dataset_extract(model):
    ...
    try:
        if os.listdir(dataset_save_path + "dataset\\"):
            ...
            x_payload_train, x_payload_test, x_payload_valid, y_train, y_test, y_valid = (
                np.load(
                    dataset_save_path + "dataset\\x_payload_train.npy",
                    allow_pickle=True,
                ),
                np.load(
                    dataset_save_path + "dataset\\x_payload_test.npy", allow_pickle=True
                ),
                np.load(
                    dataset_save_path + "dataset\\x_payload_valid.npy",
                    allow_pickle=True,
                ),
    ...
    for index in range(len(dataset_statistic)):
        print(f"index\tdataset_statistic[index]")
    print(f"all\tsum(dataset_statistic)")
    ...
    output_x_payload_test = os.path.join(
        dataset_save_path + "dataset\\", "x_payload_test.npy"
    )
...
def models_deal(
    ...
    for index in range(len(model)):
        ...
        if model[index] == "pre-train":
            ...
            print(
                f"finish generating pre-train's payload dataset.\nPlease check in {save_dir}"
            )
...
def count_label_number(samples):
    ...
    for index in range(len(dataset_length)):
        if dataset_length[index] < samples[0]:
            print(
                f"label {labels[index]} has less sample's number than defined samples {samples[0]}"
            )
    ...
    if open_dataset:
        ...
        for p, d, f in os.walk(pcap_path):
            for file in f:
                ...
                open_dataset_deal.file_2_pcap(
                    os.path.join(p, file), os.path.join(p, target_file)
                )
