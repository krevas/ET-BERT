...
def get_feature_flow(label_pcap, payload_len, payload_pac):
    ...
    for packet in packets:
        ...
        if packet_count == payload_pac:
            ...
            packet_string = data.decode()
            flow_data_string += bigram_generation(
                packet_string, packet_len=payload_len, flag=False
            )
                ...
                for p, d, f in os.walk(session_pcap_path[key]):
                    for file in f:
                        ...
                        if file_size < 5:
                            os.remove(p + "\\" + file)
                            print(
                                "remove sample: %s for its size is less than 5 KB."
                                % (p + "\\" + file)
                            )
                ...
                for p, d, f in os.walk(session_pcap_path[key]):
                    for file in f:
                        ...
                        if not os.path.getsize(current_file):
                            ...
                            file_size = float(
                                size_format(os.path.getsize(p + "\\" + file))
                            )
                            try:
                                if "TCP" in str(current_packet.res):
                                    ...
                                    if file_size < 0.14:
                                        os.remove(p + "\\" + file)
                                        print(
                                            "remove TCP sample: %s for its size is less than 0.14KB."
                                            % (p + "\\" + file)
                                        )
                                    ...
                                    if file_size < 0.1:
                                        os.remove(p + "\\" + file)
                                        print(
                                            "remove UDP sample: %s for its size is less than 0.1KB."
                                            % (p + "\\" + file)
                                        )
                            except Exception as e:
                                print(
                                    "error in data_generation 611: scapy read pcap and analyse error"
                                )
                                os.remove(p + "\\" + file)
                                print(
                                    "remove packet sample: %s for reading error."
                                    % (p + "\\" + file)
                                )
            if label_id[key] not in dataset:
